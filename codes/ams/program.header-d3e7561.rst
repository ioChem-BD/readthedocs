...
.. code:: xml
   ...
   <templateList>  <template pattern="\s*\*\s*Amsterdam\sDensity\sFunctional\s*\(ADF\)\s*development\sversion\s*\*.*" endPattern=".*">    <record id="program">\s*\*\s*Amsterdam\sDensity\sFunctional\s+\({A,cc:program}\).*</record>
               ...
               </template>  <template pattern="\s*.*RunTime:.*\s+[A-Za-z]+\s+Nodes.*" endPattern=".*" endPattern2="~" endOffset="0">    <record id="runDate">\s*.*RunTime:{X,cc:runDate}\s+[A-Za-z]+\s+Nodes.*</record>
   ...
   <transform process="createDate" xpath=".//cml:scalar[@dictRef='cc:runDate']" format="MMMdd-yyyy HH:mm:ss" />
